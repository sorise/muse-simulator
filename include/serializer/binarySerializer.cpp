...
#define MUSE_PREVENT_CROSSING_BOUNDARIES()              \
    ...
    void BinarySerializer::saveToFile(const std::string& _path) const{
        if (byteStream.empty()){
        ...
        // 首先检查整个文件路径是否合理
        std::error_code ec;
        bool isPathValid = std::filesystem::exists(file_path, ec);
        if (!ec && isPathValid) {
            // 如果路径已经存在且有效，无需创建父级目录
        }else {
            // 检查失败或路径不存在，此时我们仅关心父目录是否存在，所以获取父目录路径
            std::filesystem::path parentPath = file_path.parent_path();
            // 再次检查父路径的有效性
            isPathValid = std::filesystem::exists(parentPath, ec);
            if (!ec && isPathValid) {

            }else {
                // 如果父路径不是根目录（"." 或 "/"）且不存在，尝试创建
                if (!parentPath.empty()) {
                    try {
                        // 创建目录及其所有不存在的祖先目录
                        ...
                        //文件路径错误
                        throw std::logic_error("no right to create directory!");
                    }
                }
            }
        }
        std::ofstream saver(_path,std::ios_base::binary | std::ios_base::trunc | std::ios_base::out );
