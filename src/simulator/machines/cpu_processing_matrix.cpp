...
namespace muse::simulator{
    ...
    auto cpu_processing_matrix::set_RPC_processing_time_Server(const std::string &name,const uint32_t &_unified_cpu_down_ms,const uint32_t &_unified_cpu_up_ms) -> void {
        if (!this->initial_state_){
            throw std::logic_error("The cpu_processing_matrix is not initialized");
        }
        bool is_equal = (_unified_cpu_down_ms == _unified_cpu_up_ms);
        std::string key = name + " -> " + "trigger";
        this->cpu_map_->operator[](key) = std::make_tuple(_unified_cpu_down_ms, _unified_cpu_up_ms, is_equal);
    }

    auto cpu_processing_matrix::set_RPC_processing_time_Client(const std::string &name, const uint32_t &_unified_cpu_down_ms, const uint32_t &_unified_cpu_up_ms) -> void {
        if (!this->initial_state_){
            throw std::logic_error("The cpu_processing_matrix is not initialized");
        }
        bool is_equal = (_unified_cpu_down_ms == _unified_cpu_up_ms);
    ...
    auto cpu_processing_matrix::initial(const uint32_t &_unified_down_ms,const uint32_t &_unified_up_ms) -> bool {
        if (this->initial_state_){
            return false;
        }
        if (_unified_down_ms > _unified_up_ms){
        ...
        this->unified_cpu_down_ms_ = _unified_down_ms;
        this->unified_cpu_up_ms_ = _unified_up_ms;
        this->down_equal_up_ = (_unified_down_ms == _unified_up_ms);
    ...
    : initial_state_(false), unified_cpu_down_ms_(0), unified_cpu_up_ms_(0),down_equal_up_(true){
    ...
    auto cpu_processing_matrix::get_client(const std::string &name)->uint32_t {
        ...
        if(this->cpu_map_->find(key) == this->cpu_map_->end()){
            if (this->down_equal_up_){
                return this->unified_cpu_down_ms_;
            }else{
                return get_random_number(this->unified_cpu_down_ms_, this->unified_cpu_up_ms_);//得到0到9之间的数
            }
        }else{
