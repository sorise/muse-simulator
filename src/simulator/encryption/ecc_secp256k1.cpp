...
namespace muse::chain {
     auto ecc_secp256k1::get_key() -> std::shared_ptr<EVP_PKEY> {
        ...
        if (group == nullptr) {
            ...
            fmt::print("ecc_secp256k1::get_key openssl error -- {}", errorString);
            return nullptr;
        }
        //EC 秘钥存放上下文
    ...
    auto ecc_secp256k1::signature(EVP_PKEY* key, void *data, size_t data_size, std::string& _out_def )->bool {
        ...
        if(EVP_DigestInit(mctx, EVP_sha3_256()) <= 0){
            ...
            //写入日志
            const char *errorString = ERR_error_string(errno_, nullptr);
            fmt::print("ecc_secp256k1::signature openssl error -- {}", errorString);
            return false;
        }
        //签名进行生成
    ...
    auto ecc_secp256k1::verify(EVP_PKEY* key, void *data, size_t data_size, const std::string &in_sig_data) -> bool {
        ...
        if (EVP_DigestInit(mctx, EVP_sha3_256()) <= 0){
            ...
            //写入日志
            const char *errorString = ERR_error_string(errno_, nullptr);
            fmt::print("ecc_secp256k1::verify openssl error -- {}", errorString);
            return false;
        }
        if (EVP_DigestUpdate(mctx, data, data_size) <= 0){
            EVP_MD_CTX_free(mctx);
    ...
    auto ecc_secp256k1::store_public_key_pem(EVP_PKEY *pkey, const std::string &file_path) -> bool {
        std::filesystem::path save_path(file_path);
        if (!std::filesystem::exists( save_path.parent_path())){
            fmt::print("ecc_secp256k1::store_public_key_pem the path \"{}\" not exist!", file_path);
            return false;
        }
        ...
        if (err_pub != 0) {
            fmt::print("ecc_secp256k1::store_public_key_pem fopen_s failed");
            return false;
        }
#elif defined(__linux__)
    ...
    EVP_PKEY * ecc_secp256k1::read_public_key_from_file(const std::string &_pub_key_path) {
        ...
        if (!std::filesystem::exists( save_path)){
            fmt::print("ecc_secp256k1::read_public_key_from_file the path \"{}\" not exist!", _pub_key_path);
            return nullptr;
        }
        FILE* file_pub = nullptr;
#ifdef _WIN32
        ...
        if (err_pub != 0) {
            fmt::print("ecc_secp256k1::read_public_key_from_file fopen_s failed");
            return nullptr;
        }
#elif defined(__linux__)
    ...
    EVP_PKEY * ecc_secp256k1::read_private_key_from_file(const std::string &_pri_key_path) {
        ...
        if (!std::filesystem::exists( save_path)){
            fmt::print("ecc_secp256k1::read_private_key_from_file the path \"{}\" not exist!", _pri_key_path);
            return nullptr;
        }
        ...
        if (err_pub != 0) {
            fmt::print("ecc_secp256k1::read_private_key_from_file fopen_s failed");
            return nullptr;
        }
#elif defined(__linux__)
    ...
    auto ecc_secp256k1::convert_uint256_to_public_key(const uint256_prefix& pub_key, bool isCompressed) -> EVP_PKEY*
    {
        ...
        if (EC_KEY_set_public_key(ecKey, ecPoint) != 1) {
            fmt::print("Failed to set public key for EC_KEY");
            EC_POINT_free(ecPoint);
            EC_GROUP_free(group);
            EC_KEY_free(ecKey);
            return nullptr;
    ...
    auto ecc_secp256k1::get_public_key_hash_no_compressed(EVP_PKEY *evpPublicKey, uint256& out) -> bool {
        ...
        if (ecKey == nullptr) {
            ...
            //写入日志
            const char *errorString = ERR_error_string(errno_, nullptr);
            fmt::print("ecc_secp256k1::get_public_key_hash_no_compressed Failed to get EC_KEY from EVP_PKEY, openssl error: {}", errorString);
            return false;
        }
        ...
        if (ecPoint == nullptr) {
            ...
            //写入日志
            const char *errorString = ERR_error_string(errno_, nullptr);
            fmt::print("ecc_secp256k1::get_public_key_hash_no_compressed Failed to get EC_POINT from EC_KEY, openssl error: {}", errorString);
            EC_KEY_free(ecKey);
            return false;
        }
        ...
        if (keyLength == 0) {
            ...
            //写入日志
            const char *errorString = ERR_error_string(errno_, nullptr);
            fmt::print("ecc_secp256k1::get_public_key_hash_no_compressed Failed to get key length., openssl error: {}", errorString);
            EC_KEY_free(ecKey);
            return false;
        }
        ...
        if (EC_POINT_point2oct(EC_KEY_get0_group(ecKey), ecPoint, POINT_CONVERSION_UNCOMPRESSED, keyBuffer, keyLength, nullptr) != keyLength) {
            ...
            //写入日志
            const char *errorString = ERR_error_string(errno_, nullptr);
            fmt::print("ecc_secp256k1::get_public_key_hash_no_compressed Failed to convert EC_POINT to byte array, openssl error: {}", errorString);
            EC_KEY_free(ecKey);
            delete[] keyBuffer;
            return false;
        }
