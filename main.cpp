...
static int sign(const secp256k1_context* ctx, struct signer_secrets *signer_secrets, struct signer *signer, const secp256k1_musig_keyagg_cache *cache, const unsigned char *msg32, unsigned char *sig64) {
    ...
    for(i = 0; i < N_SIGNERS; i++){
        // 生成公钥
        secp256k1_pubkey pubkey;
        secp256k1_keypair_pub(ctx, &pubkey, &signer_secrets[i].keypair);
        ...
        unsigned char seckey[32];
        hash_handler::sha3_256(serialized_pubkey, pubkey_len, seckey);

        for (int j = 0; j < 32; ++j) {
            seckey[j] = seckey[j] ^ msg32[j];
        }
        unsigned char secret[32]; //秘密
        hash_handler::sha3_256(seckey, 32, secret);
